...
void DftUtilities::applyFilter(int v, int u, float d0, int order)
{
  ...
  for(int x = -magnitude.cols / 2; x < magnitude.cols / 2; x++)
  {
    ...
    {
      magnitude.at<float>(y + magnitude.rows / 2 , x + magnitude.cols / 2 ) *=
        butterworthFilter(x,y,v,u,d0,order);
    }
  }
}
...
double DftUtilities::pixel_distance(double u, double v)
{
    return sqrt(u * u + v * v);
}

double DftUtilities::gaussion_coefficient(double x, double y, double u, double v, double d0)
{
    double new_x = x - v;
    double new_y = y - u;
    double d = pixel_distance(new_y, new_x);
    return exp((-d * d) / (2 * d0 * d0));
}

void DftUtilities::gaussian_filter(int v, int u, double d0)
{
    for(int x = -magnitude.cols / 2; x < magnitude.cols / 2; x++)
    {
      for(int y = -magnitude.rows / 2; y< magnitude.rows / 2; y++)
      {
        magnitude.at<float>(y + magnitude.rows / 2 , x + magnitude.cols / 2 ) /=
          gaussion_coefficient(x,y,v,u,d0);
      }
    }
}
